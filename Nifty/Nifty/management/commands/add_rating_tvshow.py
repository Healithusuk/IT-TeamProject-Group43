...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for tvshow in tvshows:
            ...
            tvshow.update_rating(chosen_user.id, rating_value)
            self.stdout.write(
                self.style.SUCCESS(
                    f"电视剧 {tvshow.tvshow_name} 由用户 {chosen_user.username} 打分: {rating_value}"
                )
            )
        
        self.stdout.write(self.style.SUCCESS("所有电视剧评分已更新。"))
